...
window.auth = {
  ...
  init() {
    ...
    firebase.auth().onAuthStateChanged((user) => {
      ...
      if (user) {
        ...
        sessionStorage.setItem("netno_user_name", user.displayName || email.split('@')[0]);
  ...
  isAdmin() {
    if (this._user) return this._user.email.toLowerCase() === OWNER_EMAIL.toLowerCase();
    return this.getUserAdmin();
  },

  getUser() { return this._user; },
  getUserEmail() { return sessionStorage.getItem("netno_user_email") || ""; },
  getUserName() { return sessionStorage.getItem("netno_user_name") || ""; },
  getUserPhoto() { return sessionStorage.getItem("netno_user_photo") || ""; },
  getUserAdmin() { return sessionStorage.getItem("netno_user_admin") === "true"; },
        ...
        <div class="flex flex-col items-center gap-2 w-full max-w-xs">
          ...
          <div class="w-full mt-6 pt-6 border-t border-white/10 flex flex-col items-center gap-2">
            ${loggedIn ? `
              ...
              <a href="portal.html" class="w-full text-center font-body text-base font-semibold py-4 px-6 rounded-xl bg-primary-color text-white hover:brightness-110 transition-all duration-200 flex items-center justify-center gap-2" onclick="toggleMobileMenu()">
